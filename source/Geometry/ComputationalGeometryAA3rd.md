...
- 对变量$i$，从$3$遍历到$n$
	- 如果点$p_i$在第$i-1$个最小圆覆盖$D_{i-1}$上，则第$i$个最小圆覆盖$D_i$就是$D_{i-1}$，即$D_i = D_{i-1}$
	- 如果点$p_i$不在第$i-1$个最小圆覆盖$D_{i-1}$上，调用函数$MINIDISCWITHPOINT({p_1, p_2, \dots, p_{i-1}}, p_i)$，并将返回值赋予$D_i$，即$D_i = MINIDISCWITHPOINT({p_1, p_2, \dots, p_{i-1}}, p_i)$。
- 返回最终结果最小圆覆盖$D_n$
