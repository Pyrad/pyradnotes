...
当sweep line到达某个event point的时候，边界上就会出现一条新的边 $e$。处理的办法是，首先检查这条新的边 $e$ 是属于 $C_1$ 还是 $C_2$，再检查它是 $C_1$（或 $C_2$ ）的左侧还是右侧边界，然后调用相对的处理程序。下面以这条边 $e$ 是 $C_1$ 左侧边界上的一条边为例，说明处理过程。
