...
如果这个$(d-1)$维的线性规划程序是不可行的（infeasible），那么它的解决办法就能找到$k$个certificate half-space $h_1, h_2, \dots, h_k \in H (k < d)$，从而证明$(H, \vec{c})$是有界的。如果$k < d$，那么$(\{h_1, h_2, \dots, h_k\}, \vec{c})$的optimal solution集合就是无界的。如果是这种情况，那么这些optimal solution就构成了一个$(d-k)$维的子空间（subspace）。
